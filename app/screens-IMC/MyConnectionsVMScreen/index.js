...
import {
  getVMeetingSent,
  getVMeetingReceived,
  getVMeetingConfirmed,
  cancelVMeeting,
  acceptVMeeting,
  refuseVMeeting,
} from '../../services/vmConnectionsService';
...
export default function MyConnectionsVMScreen({navigation}) {
  ...
  const [routes] = useState(() => {
    const dynamicRoutes = [
      permissions?.includes('send_virtuel_meeting') && { key: 'sent', title: t('sent') },
      permissions?.includes('receive_virtuel_meeting') && { key: 'received', title: t('received') },
...
function SentMeetingsTab({navigation, jumpTo, refetchTrigger, isFocused}) {
  ...
  useEffect(() => {
    if (isFocused) {
      setStatus('');
      fetchVMeetingSent(pagination.current_page);
    }
  }, [refetchTrigger]);

  const [loadingCancel, setLoadingCancel] = useState(false);
  const cancelVMeetingMethod = async id => {
    setLoadingCancel(true);
    try {
      const response = await cancelVMeeting(id);
      if (response.code == 200) {
        ...
        ToastUtils.showErrorToast(
          `${t('error')}`,
          t(response?.message ?? 'cancel_meeting_error'),
        );
      }
    } catch (error) {
      setLoadingCancel(false);
  ...
  const fetchVMeetingSent = async (page, status) => {
    setMeetingSent([]);
    setLoading(true);
    try {
      const response = await getVMeetingSent(page, status);
      if (response.code == 200) {
        setLoading(false);
        ...
        <FlatList
          refreshControl={
            <RefreshControl
              ...
              refreshing={loading}
              onRefresh={() => fetchVMeetingSent(pagination.current_page)}
            />
          }
          data={meetingSent}
          ...
          (meetingItemData?.status !== 'refused' && meetingItemData?.status !== 'canceled') && (
            <View>
              <Button
                ...
                onPress={() => cancelVMeetingMethod(meetingItemData?.id)}
...
function ReceivedMeetingsTab({navigation, jumpTo, refetchTrigger, isFocused}) {
  ...
  useEffect(() => {
    if (isFocused) {
      setStatus('');
      fetchVMeetingReceived(pagination.current_page);
    }
  }, [refetchTrigger]);

  const [loadingAccept, setLoadingAccept] = useState(false);
  const acceptVMeetingMethod = async id => {
    setLoadingAccept(true);
    try {
      const response = await acceptVMeeting(id);
      if (response.code == 200) {
  ...
  const refuseVMeetingMethod = async id => {
    setLoadingRefuse(true);
    try {
      const responseRefuse = await refuseVMeeting(id);
  ...
  const fetchVMeetingReceived = async (page, status) => {
    setMeetingReceived([]);
    setLoading(true);
    try {
      const response = await getVMeetingReceived(page, status);
      if (response.code == 200) {
        setLoading(false);
  ...
  const renderItem = item => {
    return (
      <ConnectionItem
        ...
        onPressRefuse={() => refuseVMeetingMethod(item.id)}
        onPressAccept={() => acceptVMeetingMethod(item.id)}
        status={item.status}
        isExhibitor={
          type == 'exhibitor' 
  ...
  const handlePageChange = async page => {
    ...
    await fetchVMeetingReceived(page);
  };
        ...
        <FlatList
          refreshControl={
            <RefreshControl
              ...
              tintColor={colors.primary}
              refreshing={loading}
              onRefresh={() => fetchVMeetingReceived(pagination.current_page)}
            />
          }
...
function ConfirmedMeetingsTab({navigation, jumpTo, refetchTrigger, isFocused}) {
  ...
  useEffect(() => {
    if (isFocused) {
      setDate('');
      fetchVMeetingConfirmed(pagination.current_page);
    }
  }, [refetchTrigger]);

  const fetchVMeetingConfirmed = async (page, date) => {
    setMeetingConfirmed([]);
    setLoading(true);
    try {
      const response = await getVMeetingConfirmed(page, date);
      if (response.code == 200) {        
        setLoading(false);
  ...
  const handlePageChange = async page => {
    ...
    await fetchVMeetingConfirmed(page);
  };
        ...
        <FlatList
          refreshControl={
            <RefreshControl
              ...
              tintColor={colors.primary}
              refreshing={loading}
              onRefresh={() => fetchVMeetingConfirmed(pagination.current_page)}
            />
          }
