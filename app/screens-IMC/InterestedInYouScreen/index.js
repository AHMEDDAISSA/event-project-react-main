...
import {getInterstedInYouList} from '../../services/exhibitorService';
...
export default function InterestedInYouScreen({navigation}) {
  ...
  useFocusEffect(
    useCallback(() => {
      fetchInterestsInYou(pagination.current_page);
    }, []),
  );
  ...
  const fetchInterestsInYou = async (page, searchText) => {
    try {
      ...
      console.error('Error fetching interestsInYOU:', error);
      setExhibitors([]);
    } finally {
      setLoading(false);
    }
      ...
      <View style={{flex: 1}}>
        ...
        <SafeAreaView
          ...
          <TextInput
            ...
            onSubmitEditing={() => fetchInterestsInYou(1, searchText)}
            leftIcon={
              <Icon
                name="person-search"
                size={20}
